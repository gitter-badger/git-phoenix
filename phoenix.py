...
class Phoenix(abc.ABC):
    ...
    def execute(args):
        ...
        if ([] != args[0:]):
            if ("help" == args[0]):
                Logger.info(cls=Phoenix, msg=("Launching browser to show" +
                                              PythonCommons.LIGHT_RED +
                                              " Git Phoenix " +
                                              PythonCommons.NC +
                                              "help..."))
                webbrowser.open("https://github.com/victoraugustofd/git-phoenix/blob/feature/alpha-version/README.md")
            else:
